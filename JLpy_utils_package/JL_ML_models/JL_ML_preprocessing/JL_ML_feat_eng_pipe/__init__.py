...
class feat_eng_pipe():
    ...
    def save(self, X, path_feat_eng_dir, filename, format_):
        ...
        if format_ == 'h5':
            ...
            path_save_file = os.path.join(path_feat_eng_dir, filename+'.h5' )

            file = h5py.File(path_save_file, 'w')
            file.create_dataset(filename, data=X_field)
            file.close()
            ...
            path_save_file = os.path.join(path_feat_eng_dir,filename+'.csv')
            
            if type(X)==np.ndarray:
                np.savetxt(path_save_file, X, delimiter=',')
            else:
            ...
            import json
            path_save_file = os.path.join(path_feat_eng_dir, filename+'.json')
            ...
            import dill
            path_save_file = os.path.join(path_feat_eng_dir, filename+'.dill')
    ...
    def load(self, path_feat_eng_dir, filename, format_, header='infer'):
        ...
        if format_ == 'h5': 
            ...
            path_save_file = os.path.join(path_feat_eng_dir, filename+'.csv')
            X = pd.read_csv(path_save_file, low_memory = False, header = header)
    ...
    def __run_OneHot_Encode__(self,
        ...
        if self.__feat_eng_files_saved__(path_feat_eng_dir, format_, find_X_field)==False or self.overwrite==True or self.overwrite=='OneHot' or self.overwrite == 'OneHotEncode' or self.overwrite == 'OneHot_Encode':
            ...
            if OneHot_case:
                ...
                self.LabelEncoder = self.load(os.path.join(self.path_feat_eng_root_dir, 'LabelEncode'), 'LabelEncoder', 'dill')
                ...
                #save the encoder
                self.save(OneHotEncoder, path_feat_eng_dir, 'OneHotEncoder', 'dill')
